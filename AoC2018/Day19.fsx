...
let runProgram pointerReg instructions =
    ...
    let unfolder (r: Map<int,int>) =
        let pointer  = r.[pointerReg]
        if pointer < 0 || pointer >= upper then
            None
        else
            ...
            let msg = sprintf "%s: %A" (pointer.ToString().PadLeft(2,'0')) r
            System.Console.WriteLine msg
            Some(finalR,finalR)
    Seq.unfold unfolder (register.Remove(0).Add(0,1))
    |> Seq.last
...
runProgram 0 testprogram

let part1 = 
    runProgram 3 commands

let part2 = 
 [1..3500] |> List.filter(fun t -> 10551300%t = 0) |> List.collect(fun t -> [t;10551300/t])|> List.sum